...
class ProjectDataManagement:
    ...
    def get_project(project_id, db):
        project_doc = db.collection(u'projects').document(project_id).get()
    ...
    def get_quotes(project_id, db):
        ...
        for quote in quotes:
            ...
            total_price = sum(map(lambda item: float(item['item_unit_price']) * float(item['item_number']), items))
